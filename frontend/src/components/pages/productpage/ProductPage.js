...
const ProductPage = () => {
  ...
  useEffect(() => {
    ...
    if (cataId && !typeId) {
      ...
      let linkAPI = `http://localhost:8000/api/product/cata/${cataId}/${temp}`;
      axios
        .get(linkAPI)
        .then((res) => {
      ...
      let linkAPI = `http://localhost:8000/api/product/type/${typeId}/${temp}`;
      axios
        .get(linkAPI)
        .then((res) => {
          setDataProdGroup(res.data);
      ...
      let tempp =
        chooseFilter === 1
          ...
          ? "sort/asc"
          : "sort/desc";
      let linkAPI = `http://localhost:8000/api/product/${tempp}`;
      axios
        .get(linkAPI)
        .then((res) => {
  ...
  useEffect(() => {
    ...
    if (priceTo && priceFrom) {
      setLoadingProd(true);
      let linkAPI = `http://localhost:8000/api/product/price/${priceFrom}/${priceTo}`;
      axios
        .get(linkAPI)
        .then((res) => {
